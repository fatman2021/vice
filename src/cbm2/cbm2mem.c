...
{
    // added check for *_ptr variables: they could be uninitialized;
    // e.g. DO_INTERRUPT() in 6510core.c handling IK_TRAP|IK_RESET at startup;
    // IK_TRAP (which calls this function) is handled before handling IK_RESET
    // (which calls mem_powerup() and initializes *_ptr variables)
    BYTE *p = _mem_read_base_tab_ptr ? _mem_read_base_tab_ptr[addr >> 8] : NULL;

    *base = (p == NULL) ? NULL : (p - (addr & 0xff00));
    *start = addr; /* TODO */
    *limit = mem_read_limit_tab_ptr ? mem_read_limit_tab_ptr[addr >> 8] : 0;
}
...
*/
BYTE mem_read_screen(WORD addr)
{
 ...
 */
void mem_get_cursor_parameter(WORD *screen_addr, BYTE *cursor_column, BYTE *line_length, int *blinking)
{
