...
{
    WORD screen_addr, addr;
    BYTE line_length, cursor_column;
    int i, blinking;
    ...
    for (i = 0; s[i] != '\0'; i++) {
        if (mem_read_screen((WORD)(addr + i) & 0xffff) != s[i] % 64) {
            if (mem_read_screen((WORD)(addr + i) & 0xffff) != (BYTE)32) {
 ...
 */
static void restore_drive_emulation_state(void)
{
...
/* Reset autostart.  */
void autostart_reinit(CLOCK min_cycles_, int handle_tde, int _blnsw, int _pnt, int _pntr, int _lnmx)
{
...
int autostart_init(CLOCK min_cycles_, int handle_drive_true_emulation, int blnsw, int pnt, int pntr, int lnmx)
{
    autostart_prg_init();

    autostart_reinit(min_cycles_, handle_drive_true_emulation, blnsw, pnt, pntr, lnmx);
...
static void setup_for_disk(void)
{
    if (handle_drive_true_emulation_overridden) {
