...
int cmdline_parse(int *argc, char **argv)
{
...
#ifdef WIN32 /* RetroPlatform */
            /* RetroPlatform command line options are private (not registered with cmdline_register_options())
               since they are just meant to programmatically start the emulator as a RetroPlatform guest */
            if (strcmp(argv[i], "-rphost") == 0) {
                hr = retroplatform_initialize(argv[i+1]);
				if (FAILED(hr)) {
	                archdep_startup_log_error("RetroPlatform initialization failed (error 0x%X).\n", hr);
                    return -1;
                }
                i += 2;
                continue;
            }
            if (strcmp(argv[i], "-rpscreenmode") == 0) {
                set_retroplatform_screen_mode(argv[i+1]);
                i += 2;
                continue;
            }
            if (strcmp(argv[i], "-rpenabledtapes") == 0) {
                set_retroplatform_enabled_tapes(argv[i+1]);
                i += 2;
                continue;
            }
            if (strcmp(argv[i], "-rpdatapath") == 0) { /* handled in archdep.c */
                i += 2;
                continue;
            }
			if (strcmp(argv[i], "-rpdpiawareness") == 0) {
                set_retroplatform_dpiawareness(argv[i+1]);
                i += 2;
                continue;
			}
			if (strcmp(argv[i], "-rpvirtualmouse") == 0) {
                enable_retroplatform_virtualmouse(argv[i+1]);
                i += 2;
                continue;
			}
			if (strcmp(argv[i], "-rpscale2x") == 0) {
                set_retroplatform_scale2x(argv[i+1]);
                i += 2;
                continue;
			}
#endif
    ...
    *argc = (int)j;
    ...
    {
        const char *log_file;
        if (resources_get_string("LogFileName", &log_file) >= 0) {
            if (log_file == NULL || *log_file == 0) { /* disables logging if launched as RetroPlatform guest (unless a log file has been specified on the command line) */
                resources_set_int("Log", 0);
            }
        }
    }
#endif
