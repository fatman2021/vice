...
static void drive_led_update(drive_t *drive, drive_t *drive0)
{
    ...
    old_led_flags = drive->led_flags;
    if (drive->read_write_mode == 0 && !(old_led_flags & LEDFLAG_WRITE_MODE_DISPLAYED))
        drive->led_flags |= LEDFLAG_WRITE_MODE;

    if (led_pwm != drive->led_last_pwm
        || my_led_status != drive->old_led_status
        || drive->led_flags != old_led_flags) {
        ui_display_drive_led(drive->mynumber, led_pwm,
                             (my_led_status & 2) ? 1000 : 0,
                             (drive->led_flags & LEDFLAG_WRITE_MODE) ? 1 : 0);
        if (led_pwm >= 500) {
            if (drive->led_flags & LEDFLAG_WRITE_MODE)
                drive->led_flags |= LEDFLAG_WRITE_MODE_DISPLAYED;
        }
		else if (drive->led_flags & LEDFLAG_WRITE_MODE_DISPLAYED) {
            drive->led_flags = 0;  /* the write mode flag stays set until the led is turned off */
        }
