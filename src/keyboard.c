...
static key_alarm_func_t keyboard_alarm_func;
...
static void keyboard_latch_handler(CLOCK offset, void *data)
{
    ...
    if (keyboard_alarm_func) {
        kaf = keyboard_alarm_func;
        keyboard_alarm_func = NULL;
        kaf();
    }
}
...
int keyboard_restore_pressed(key_alarm_func_t kaf)
{
    DWORD event_data;
    int alarm = 0;
    event_data = (DWORD)1;
...
int keyboard_restore_released(key_alarm_func_t kaf)
{
    DWORD event_data;
    int alarm = 0;
    event_data = (DWORD)0;
...
int keyboard_do_key_pressed_matrix(int row, int column, key_alarm_func_t kaf)
{
    int alarm = 0;
	int shift = ALLOW_SHIFT;

	if (row == kbd_lshiftrow && column == kbd_lshiftcol)
		shift |= LEFT_SHIFT;
	else if (row == kbd_rshiftrow && column == kbd_rshiftcol)
		shift |= RIGHT_SHIFT;
...
void keyboard_key_released(signed long key)
{
    ...
    if (((key == key_ctrl_restore1) || (key == key_ctrl_restore2))
        && machine_has_restore_key()) {
        keyboard_restore_released(NULL);
        return;
    }
...
int keyboard_do_key_released_matrix(int row, int column, key_alarm_func_t kaf)
{
    int alarm = 0;
	int shift = ALLOW_SHIFT;
    ...
    if (keyboard_key_released_matrix(row, column, shift)) {
