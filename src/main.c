...
int main_program(int argc, char **argv)
{
    ...
    */
    ...
    /* log_init() has been moved after the initcmdline_check_args() call:
       the default log file doesn't get created if logging is disabled
       or if a different log file has been specified on the command line
       (early debug output goes to archdep_default_logger() only) */
    if (log_init() < 0) {
