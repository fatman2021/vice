...
typedef HANDLE (WINAPI *PFN_PowerCreateRequest)(PREASON_CONTEXT Context);
typedef BOOL (WINAPI *PFN_PowerSetRequest)(HANDLE PowerRequest, POWER_REQUEST_TYPE RequestType);
typedef BOOL (WINAPI *PFN_PowerClearRequest)(HANDLE PowerRequest, POWER_REQUEST_TYPE RequestType);
struct ScreenSaverData
{
	HMODULE hKernel32;
	HANDLE hRequest;
	PFN_PowerCreateRequest pfnPowerCreateRequest;
	PFN_PowerSetRequest pfnPowerSetRequest;
	PFN_PowerClearRequest pfnPowerClearRequest;
	REASON_CONTEXT Context;
	BOOL bScreenSaveActive;
	BOOL bDisabled;
};
static struct ScreenSaverData g_ScreenSaverData;

static void initialize_screen_saver_data(void)
{
	memset(&g_ScreenSaverData, 0, sizeof(g_ScreenSaverData));
	g_ScreenSaverData.hRequest = INVALID_HANDLE_VALUE;
	g_ScreenSaverData.hKernel32 = LoadLibrary(_T("Kernel32.dll"));
	if (g_ScreenSaverData.hKernel32)
	{
		g_ScreenSaverData.pfnPowerCreateRequest = (PFN_PowerCreateRequest)GetProcAddress(g_ScreenSaverData.hKernel32, "PowerCreateRequest");
		g_ScreenSaverData.pfnPowerSetRequest = (PFN_PowerSetRequest)GetProcAddress(g_ScreenSaverData.hKernel32, "PowerSetRequest");
		g_ScreenSaverData.pfnPowerClearRequest = (PFN_PowerClearRequest)GetProcAddress(g_ScreenSaverData.hKernel32, "PowerClearRequest");
		if (g_ScreenSaverData.pfnPowerCreateRequest &&
			g_ScreenSaverData.pfnPowerSetRequest &&
			g_ScreenSaverData.pfnPowerClearRequest)
		{
			g_ScreenSaverData.Context.Version = POWER_REQUEST_CONTEXT_VERSION;
			g_ScreenSaverData.Context.Flags = POWER_REQUEST_CONTEXT_SIMPLE_STRING;
			g_ScreenSaverData.Context.Reason.SimpleReasonString = L"VICE Emulation";
			g_ScreenSaverData.hRequest = g_ScreenSaverData.pfnPowerCreateRequest(&g_ScreenSaverData.Context);
		}
	}
}

static void disable_screen_saver(BOOL bDisable)
{
	if (g_ScreenSaverData.hKernel32)
	{
		if (bDisable && !g_ScreenSaverData.bDisabled)
		{
			if (g_ScreenSaverData.hRequest != INVALID_HANDLE_VALUE)
			{
				if (!g_ScreenSaverData.pfnPowerSetRequest(g_ScreenSaverData.hRequest, PowerRequestDisplayRequired))
				{
					CloseHandle(g_ScreenSaverData.hRequest);
					g_ScreenSaverData.hRequest = INVALID_HANDLE_VALUE;
				}
			}
			if (g_ScreenSaverData.hRequest == INVALID_HANDLE_VALUE)
			{
				SystemParametersInfo(SPI_GETSCREENSAVEACTIVE, 0, &g_ScreenSaverData.bScreenSaveActive, 0);
				if (g_ScreenSaverData.bScreenSaveActive)
					SystemParametersInfo(SPI_SETSCREENSAVEACTIVE, FALSE, NULL, SPIF_SENDWININICHANGE);
			}
			g_ScreenSaverData.bDisabled = TRUE;
		}
		else if (!bDisable && g_ScreenSaverData.bDisabled)
		{
			if (g_ScreenSaverData.hRequest != INVALID_HANDLE_VALUE)
			{
				g_ScreenSaverData.pfnPowerClearRequest(g_ScreenSaverData.hRequest, PowerRequestDisplayRequired);
			}
			else
			{
				if (g_ScreenSaverData.bScreenSaveActive)
			...
			g_ScreenSaverData.bDisabled = FALSE;
		}
	}
}

static void uninitialize_screen_saver_data(void)
{
	disable_screen_saver(FALSE);
	if (g_ScreenSaverData.hRequest != INVALID_HANDLE_VALUE)
		CloseHandle(g_ScreenSaverData.hRequest);
	if (g_ScreenSaverData.hKernel32)
		FreeLibrary(g_ScreenSaverData.hKernel32);
	memset(&g_ScreenSaverData, 0, sizeof(g_ScreenSaverData));
	g_ScreenSaverData.hRequest = INVALID_HANDLE_VALUE;
}
...
{
    ui_accelerator = retroplatform_guest_mode ? NULL : uikeyboard_create_accelerator_table();
    ui_fullscreen_init();
	initialize_screen_saver_data();
    atexit(ui_exit);
    return 0;
}
...
void ui_exit(void)
{
	...
	uninitialize_screen_saver_data();
...
void ui_open_canvas_window(video_canvas_t *canvas)
{
    ...
    if (retroplatform_guest_mode) {
        dwStyle = WS_POPUP; /* if run as a RetroPlatform guest, open a borderless owned window */
        hParent = (HWND)retroplatform_parent();
    }
    else {
        dwStyle = WS_CAPTION | WS_CLIPCHILDREN | WS_BORDER | WS_DLGFRAME | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
        hParent = NULL;
    }
    ...
    if (retroplatform_guest_mode) {
        SetMenu(hwnd, NULL);  /* remove the menu here, not at class-creation time (when retroplatform_guest_mode is not set yet) */
        /* a call to retroplatform_startup() - in main() - */
        /* will take care of initializing and showing the window */
    }
    else {
...
void ui_canvas_child_window(video_canvas_t *canvas, int enable)
{
    if (canvas->client_hwnd == NULL) {
        canvas->client_hwnd = CreateWindowEx(WS_EX_NOPARENTNOTIFY, 
                                             ...
                                             NULL);
        if (retroplatform_guest_mode) {
            SetMenu(canvas->client_hwnd, NULL);  /* remove the menu here, not at class-creation time (when retroplatform_guest_mode is not set yet) */
        }
    }
...
//    if (IsFullscreenEnabled()) { return; }
    ...
    do {
        ...
        if (!retroplatform_guest_mode) {
            wrect.bottom += statusbar_get_status_height();
        ...
        if (place.showCmd == SW_SHOWNORMAL) {
            ...
            if (retroplatform_guest_mode && retroplatform_started_up && w == retroplatform_active_window) {
                set_retroplatform_window(w, window_index);  /* post-initialization resize */
            }
        }
        else {
...
{
    if (retroplatform_guest_mode) {
        retroplatform_report_turbo_cpu();
    }
}
...
static void update_menus(HWND hwnd)
{
    ...
    if (!menu) { /* no menu is associated to the window when in RetroPlatform guest mode */
        return;
    }
...
static void pause_trap(WORD addr, void *data)
{
    ...
    vsync_suspend_speed_eval();
    while (is_paused) {
		if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
			ui_dispatch_events();  // fixes issue of delayed WM_LBUTTONUPs
		}
		else {
			Sleep(10);
...
void ui_display_speed(float percent, float framerate, int warp_flag)
{
    ...
    for (index = 0; index < number_of_windows; index++) {
        ...
        if (!retroplatform_guest_mode) { /* a RetroPlatform guest should not change its window title */
...
void ui_enable_drive_status(ui_drive_enable_t enable, int *drive_led_color)
{
    ...
    if (retroplatform_guest_mode) {
        retroplatform_enable_drive_status(enable);
    }
}
...
void ui_display_drive_led(int drivenum, unsigned int led_pwm1, unsigned int led_pwm2, int write_mode)
{
    new_pwm1[drivenum] = led_pwm1;
    if (retroplatform_guest_mode) {
        retroplatform_display_drive_led(drivenum, led_pwm1, write_mode);
    }
}
...
void ui_display_paused(int flag)
{
    ...
    for (index = 0; index < number_of_windows; index++) {
        ...
        if (machine_class != VICE_MACHINE_VSID &&
            !retroplatform_guest_mode) { /* a RetroPlatform guest should not change its window title */
            SetWindowText(window_handles[index], st_buf);
        }
    }
}
...
void ui_dispatch_next_event(void)
{
    ...
    if (ui_accelerator) {
		...
		if (retroplatform_guest_mode && (msg.message == WM_SYSKEYDOWN || msg.message == WM_SYSKEYUP) && msg.wParam == VK_ESCAPE) { // we don't want <Esc> to exit from (hardware) fullscreen
            return;
		}
        TranslateMessage(&msg);
        DispatchMessage(&msg);
...
static bool ui_get_machine_specific_toggle(int item_id, int *value_return)
{
    int i;
    if (machine_specific_toggles) {
        for (i = 0; machine_specific_toggles[i].name != NULL; i++) {
            if (machine_specific_toggles[i].item_id == item_id) {
                resources_get_int(machine_specific_toggles[i].name, value_return);
                return TRUE;
            }
        }
    }
...
int ui_get_double_size(void)
{
    int value;
    return ui_get_machine_specific_toggle(IDM_TOGGLE_DOUBLESIZE, &value) ? (value ? 1 : 0) : -1;
}

int ui_get_double_scan(void)
{
    int value;
    return ui_get_machine_specific_toggle(IDM_TOGGLE_DOUBLESCAN, &value) ? (value ? 1 : 0) : -1;
}

int ui_get_full_screen(void)
{
    return IsFullscreenEnabled();
}

void ui_toggle_double_size(void)
{
    handle_default_command(IDM_TOGGLE_DOUBLESIZE, 0, ui_get_main_hwnd());
}

void ui_toggle_double_scan(void)
{
    handle_default_command(IDM_TOGGLE_DOUBLESCAN, 0, ui_get_main_hwnd());
}

void ui_toggle_full_screen(void)
{
    handle_wm_command(IDM_TOGGLE_FULLSCREEN, 0, ui_get_main_hwnd());
}

void ui_toggle_full_screen_win(HWND window)
{
    handle_wm_command(IDM_TOGGLE_FULLSCREEN, 0, window);
}

void ui_cart_freeze(void)
{
    handle_wm_command(IDM_CART_FREEZE, 0, ui_get_main_hwnd());
}
...
void ui_silent_exit(void)
{
    resources_set_int("ConfirmOnExit", 0);
    resources_set_int("SaveResourcesOnExit", 0);
    ui_wm_close(ui_get_main_hwnd());
}

void ui_switch_display(void)
{
    if (machine_class == VICE_MACHINE_C128 && !ui_get_full_screen()) {
        RECT rc;
        HWND show = (retroplatform_active_window == window_handles[0]) ? window_handles[1] : window_handles[0];
        HWND hide = (retroplatform_active_window == window_handles[0]) ? window_handles[0] : window_handles[1];
        GetWindowRect(hide, &rc);
        SetWindowPos(show, hide, rc.left, rc.top, 0,0, SWP_NOSIZE | SWP_SHOWWINDOW);
        ShowWindow(hide, SW_HIDE);
		if (retroplatform_guest_mode) {
			set_retroplatform_window(show, (show == window_handles[0]) ? 0 : 1);
		}
    }
}
...
static LRESULT CALLBACK window_proc(HWND window, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (msg) {
        ...
        case WM_ENABLE:
            if (retroplatform_guest_mode) {
                retroplatform_wm_enable(wparam);
            }
            break;
        case WM_POWERBROADCAST:
            ...
            break;
        case WM_SHOWWINDOW:
            if (retroplatform_guest_mode && wparam != 0 && lparam == 0) {
                retroplatform_wm_activateapp(1, 0); // fixes activation issue (we don't receive WM_ACTIVATEAPP at startup)
            }
            break;
        case WM_ACTIVATEAPP:
            if (retroplatform_guest_mode) {
                retroplatform_wm_activateapp(wparam, lparam);
            }
            break;
        case WM_ACTIVATE:
            ...
            if (retroplatform_guest_mode) {
                retroplatform_wm_activate(wparam, lparam);
            }
            break;
        case WM_SIZE:
            if (window_index<number_of_windows && !retroplatform_guest_mode) {
        ...
        case WM_NCCALCSIZE:
            if (retroplatform_guest_mode) {
                LRESULT lr;
                if (retroplatform_wm_nccalcsize((void *)window, wparam, lparam, &lr)) {
                    return lr;
                }
            }
            break;
        case WM_NCPAINT:
            if (retroplatform_guest_mode) {
                if (retroplatform_wm_ncpaint((void *)window, wparam, lparam)) {
                    return 0;
                }
            }
            break;
        ...
        case WM_NCLBUTTONDOWN:
            if (retroplatform_guest_mode && msg == WM_SYSCOMMAND && (wparam & 0xFFF0) == SC_CLOSE) { /* handle ALT+F4 */
                retroplatform_wm_close();
                return 0;
            }
            vsync_suspend_speed_eval();
            break;
        ...
        case WM_MOUSELEAVE:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_MOUSELEAVE, wparam, lparam);
			}
            break;
        case WM_MOUSEMOVE:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_MOUSEMOVE, wparam, lparam);
			}
            break;
        case WM_LBUTTONDOWN:
			if (retroplatform_guest_mode) {
				if (retroplatform_send_mouse_events) {
					retroplatform_mouse_event(window, WM_LBUTTONDOWN, wparam, lparam);
				}
				else if (!retroplatform_mouse && !_mouse_enabled && !ui_emulation_is_paused()) {
	                retroplatform_lbuttondown(wparam, lparam);
				}
			}
            break;
        case WM_LBUTTONUP:
			if (retroplatform_guest_mode) {
				if (retroplatform_send_mouse_events) {
					retroplatform_mouse_event(window, WM_LBUTTONUP, wparam, lparam);
				}
				else if (retroplatform_mouse && !_mouse_enabled && !ui_emulation_is_paused()) {
        ...
        case WM_RBUTTONDOWN:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_RBUTTONDOWN, wparam, lparam);
			}
        ...
        case WM_RBUTTONUP:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_RBUTTONUP, wparam, lparam);
			}
            break;
        case WM_MBUTTONDOWN:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_MBUTTONDOWN, wparam, lparam);
			}
            break;
        case WM_MBUTTONUP:
			if (retroplatform_guest_mode && retroplatform_send_mouse_events) {
				retroplatform_mouse_event(window, WM_MBUTTONUP, wparam, lparam);
			}
            break;
#endif
        case WM_NOTIFY:
            ...
            break;
        case WM_NCACTIVATE:
            if (IsFullscreenEnabled() && fullscreen_get_nesting_level() == 0 && !retroplatform_guest_mode) { /* Alt-Tab can be used to exit from fullscreen, if retroplatform_guest_mode */
                return 0;
            }
            break;
        case WM_SETCURSOR:
            /* the ShowCursor()-based approach to hiding the cursor
               has been replaced with a WM_SETCURSOR-based one
               (which only affects the emulator window and not other windows,
               like the RetroPlatform player window, when VICE runs as a guest) */
            if (cursor_active < 0) {
                SetCursor(NULL);
                return TRUE;
            }
