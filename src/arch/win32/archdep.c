...
static char *argv0;
static char *retroplatform_datapath;
...
int archdep_init(int *argc, char **argv)
{
    ...
    retroplatform_datapath = NULL;
    for (i = 1; i < *argc; i++) {
        if (strcmp(argv[i], "-rpdatapath") == 0) {
            retroplatform_datapath = lib_stralloc(argv[i+1]);
            break;
        }
    }
    return 0;
...
char *archdep_default_sysfile_pathlist(const char *emu_id)
{
    ...
    char c64fd[MAX_PATH+1];
    char *c64f_relative_shared, *default_path2, *pc;
    DWORD n;
    ...
    if (default_path == NULL) {
        ...
        if (default_path) {
            if (retroplatform_datapath) {
                default_path2 = util_concat(retroplatform_datapath, "\\Shared\\rom", ARCHDEP_FINDPATH_SEPARATOR_STRING,
                                            default_path, NULL);
                lib_free(default_path);
                default_path = default_path2;
            }
            else { /* legacy RetroPlatform path */
                c64f_relative_shared = util_concat(boot_path, "\\..\\..\\..\\CBM Files\\Shared\\rom", NULL);
                if (c64f_relative_shared) {
                    if (GetFileAttributes(c64f_relative_shared) != (DWORD)-1) {
                        default_path2 = util_concat(c64f_relative_shared, ARCHDEP_FINDPATH_SEPARATOR_STRING,
                                                    default_path, NULL);
                        lib_free(default_path);
                        default_path = default_path2;
                    }
                    else {
                        n = ExpandEnvironmentStrings("%C64FOREVERDATA%", c64fd, MAX_PATH);
                        if (n > 0 && n < MAX_PATH && GetFileAttributes(c64fd) != (DWORD)-1) {
                            pc = &c64fd[strlen(c64fd)-1];
                            if (*pc == '\\')
                                *pc = 0;
                            default_path2 = util_concat(c64fd, "\\Shared\\rom", ARCHDEP_FINDPATH_SEPARATOR_STRING,
                                                        default_path, NULL);
                            lib_free(default_path);
                            default_path = default_path2;
                        }
                    }
                    lib_free(c64f_relative_shared);
                }
            }
        }
    }
...
void archdep_shutdown(void)
{
    lib_free(boot_path);
    lib_free(argv0);
    free(orig_workdir); // don't use lib_free(): it's been allocated by getcwd()
    if (retroplatform_datapath) {
        lib_free(retroplatform_datapath);
    }
