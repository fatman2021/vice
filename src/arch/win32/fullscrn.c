...
static void fullscreen_getmodes_dx9(void)
{
    ...
    while (D3D_OK == IDirect3D9_GetAdapterIdentifier(d3d, numAdapter, 0, &d3didentifier)) {
		...
		hm = IDirect3D9_GetAdapterMonitor(d3d, numAdapter);
        GetMonitorInfo(hm, (LPMONITORINFO)&mi);
        new_device->rect = mi.rcMonitor;
        if (devices == NULL) {
            devices = new_device;
        } else {
...
#ifdef HAVE_D3D9_H
int select_best_fullscreen_mode(const RECT *monitor_rect, int width, int height, use_tv_modes tvm)
{
    DirectDrawModeList *mode, *best_mode;
    int delta_size, best_delta_size, best_depth, device, d;
    HMONITOR hm;
    MONITORINFOEX mi;

	device = D3DADAPTER_DEFAULT;
    if (video_dx9_enabled()) {
		if (!monitor_rect)
			return 0;
		for (d = 0; ; d++) { /* device #0: primary monitor; devices #1..n: remaining monitors */
			hm = IDirect3D9_GetAdapterMonitor(d3d, d);
			if (hm == NULL)
				break;
	        mi.cbSize = sizeof(mi);
	        if (GetMonitorInfo(hm, (LPMONITORINFO)&mi)) {
				if (mi.rcMonitor.left == monitor_rect->left &&
					mi.rcMonitor.top == monitor_rect->top &&
					mi.rcMonitor.right == monitor_rect->right &&
					mi.rcMonitor.bottom == monitor_rect->bottom) {
					device = d;
					if ((mi.rcMonitor.right - mi.rcMonitor.left) >= width && 
						(mi.rcMonitor.bottom - mi.rcMonitor.top) >= height) {
						width = mi.rcMonitor.right - mi.rcMonitor.left; /* use native monitor resolution (better stretch/scanlines) */
	...
	for (mode = modes; mode != NULL; mode = mode->next) {
        if (mode->devicenumber == device) {
            if (mode->width < width || mode->height < height)
                continue;
            if (mode->width == 720 && (mode->height == 480 || mode->height == 576)) { /* a TV mode */
                if (tvm == utvm_never)
                    continue;
            }
            else if (tvm == utvm_always)
                continue;
            delta_size = (mode->width - width) + (mode->height - height);
            if (delta_size < best_delta_size ||
                (delta_size == best_delta_size &&
                 mode->bitdepth <= 32 && mode->bitdepth > best_depth))
            {
                best_mode = mode;
                best_delta_size = delta_size;
                best_depth = mode->bitdepth;
            }
        }
    }
    if (!best_mode) {
    ...
    log_verbose("Fullscreen for %dx%d canvas: device %d, width %d, height %d.", width, height, best_mode->devicenumber, best_mode->width, best_mode->height);
    resources_set_int("FullscreenDevice", best_mode->devicenumber);
    resources_set_int("FullscreenWidth", best_mode->width);
    resources_set_int("FullscreenHeight", best_mode->height);
    resources_set_int("FullscreenRefreshRate", best_mode->refreshrate);
    return 1;
}
#endif
