...
int c64rom_load_kernal(const char *rom_name, BYTE *cartkernal)
{
    ...
    if (c64rom_get_kernal_chksum_id(&sum, &id) < 0) {
        ...
        /* rev = id; */ /* use what was previously stored in "KernalRev" (e.g. SX-64 KernalRev=67 set from INI file, when ROM was not yet loaded; the resources_set_int() call below applies patches as needed) */
    }
    if (machine_class != VICE_MACHINE_C64DTV) {
